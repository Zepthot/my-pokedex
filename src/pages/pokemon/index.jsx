...
import { Spinner, Badge, Button, Tab, Tabs, Table, Placeholder } from 'react-bootstrap';
...
function Pokemon () {
    ...
    console.log('pkmn loading: ', loading);
    ...
    let totalStats = 0;
    return (
        <div className={`pkmn ${loading ? `normal-light` : `${data.types[0].type.name}-light`}`}>
            <div className='pkmn__header'>
                ...
                <div style={{display: 'flex'}}>
                    <div className="pkmn__header__images">
                        ...
                        {loading ? <Spinner className='pkmn__header__images__sprite__loading' /> :
                    ...
                    <div className='pkmn__header__infos'>
                        <div className='pkmn__header__infos__number__star'>
                            ...
                            <Button variant='' className='pkmn__header__infos__number__star__button' onClick={() => setShiny(!isShiny)}>
                                <FaStar style={isShiny ? {color: "gold"} : {color: "ghostwhite"}} />
                            </Button>
                        </div>
                        {loading ? <Placeholder md={12} size='lg' className='pkmn__header__infos__name__loading' /> : <h1 className='pkmn__header__infos__name'>{data.name.charAt(0).toUpperCase()}{data.name.slice(1)}</h1> }
                        <div className='pkmn__header__infos__types'>
                            {loading ? <Badge className={`pkmn__header__infos__types__badge normal`} bg=""><Placeholder md={10} /></Badge> :
                            <>
            ...
            <Tabs activeKey={key} onSelect={(k) => setKey(k)} className={`mb-3 ${loading ? `normal` : `${data.types[0].type.name}`}`}>
                <Tab eventKey='about' title='About' className={`pkmn__tab ${loading ? `normal` : `${data.types[0].type.name}`}`}>
                        ...
                        <Table borderless className='pkmn__tab__section__table'>
                            ...
                            <tbody>
                                <tr>
                                    <td>Category</td>
                                    {loading ? 
                                    <td><Placeholder md={5} /></td> :
                                    <td>{dataspec.genera.map((element, index) => {
                                        if (element.language.name === 'en') {
                                            return (
                                                <Fragment key={index}>{dataspec.genera[index].genus}</Fragment>
                                            )
                                        } else {
                                            return null;
                                        }
                                ...
                                <tr>
                                    ...
                                    <td>
                                        {loading ?
                                            <ol>
                                                <li><Placeholder md={4} /></li>
                                                <li><Placeholder md={6} /></li>
                                            </ol> :
                                        <ol>
                                ...
                                <tr>
                                    <td>Hatch time</td>
                                    {loading ?
                                    <td><Placeholder md={2} /> - <Placeholder md={4} /></td> :
                                    <td>{dataspec.hatch_counter} cycles - {dataspec.hatch_counter * 128} steps</td> }
                                </tr>
                                <tr>
                                    <td>EV yield</td>
                                    {loading ?
                                            <td><Placeholder md={6} /></td> :
                                    <td className='pkmn__tab__section__table__EV'>
                                        {/* eslint-disable-next-line */}
                                        {data.stats.map((stat, index) => {
                                            if(stat.effort > 0) {
                                                return (
                                                    <span key={index}>+{stat.effort} {stat.stat.name.charAt(0).toUpperCase()}{stat.stat.name.slice(1)}</span>
                                                )
                                            }
                                        })}
                                    </td>}
                ...
                <Tab eventKey='stats' title='Statistics' className={`pkmn__tab ${loading ? `normal` : `${data.types[0].type.name}`}`}>
                        ...
                        <Table borderless className='pkmn__tab__section__table'>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td><Placeholder md={6} /></td>
                                        <td><Placeholder md={2} /></td>
                                        <td><Placeholder md={7} /></td>
                                    </tr>
                                    <tr>
                                        <td><Placeholder md={6} /></td>
                                        <td><Placeholder md={2} /></td>
                                        <td><Placeholder md={3} /></td>
                                    </tr>
                                    <tr>
                                        <td><Placeholder md={3} /></td>
                                        <td><Placeholder md={2} /></td>
                ...
                {!loading && dataevo.chain.evolves_to.length !== 0 && 
                    <Tab eventKey='evolution' title='Evolution' className={`pkmn__tab ${data.types[0].type.name}`}>
                        <section id='evolution' className='pkmn__tab__section pkmn__evolution'>
                            <div className='pkmn__evolution__container'>
                                <CardEvo url={dataevo.chain.species.url.split('/')[6]} name={dataevo.chain.species.name} shiny={isShiny} />
                                <div className='pkmn__evolution__container__treeEvo'>
                                    ...
                                    {dataevo.chain.evolves_to.map((first, index) => {
                                        return (
                                            <div key={index} className='pkmn__evolution__container__treeEvo__column'>
                                                <div className='pkmn__evolution__container__treeEvo__column'>
                                                    <TriggerEvo evo={first.evolution_details[first.evolution_details.length - 1]} />
                                                    <CardEvo url={first.species.url.split('/')[6]} name={first.species.name} shiny={isShiny} />
                                                </div>
                                                <div className='pkmn__evolution__container__treeEvo'>
                                                    {first.evolves_to.map((second, ind) => {
                                                        return (
                                                            <div key={ind} className='pkmn__evolution__container__treeEvo__column'>
                                                                <TriggerEvo evo={second.evolution_details[second.evolution_details.length - 1]} />
                                                                <CardEvo url={second.species.url.split('/')[6]} name={second.species.name} shiny={isShiny} />
                                                            </div>
                                                        )
                                                    })}
                                                </div>
                ...
                <Tab eventKey='gallery' title='Gallery' className={`pkmn__tab ${loading ? `normal` : `${data.types[0].type.name}`}`}>
                    <section id='gallery' className={`pkmn__tab__section ${loading ? `pkmn__gallery` : null}`}>
                        {loading ? <Spinner className='pkmn__gallery__loading' /> :
                        <CarouselGallery 
