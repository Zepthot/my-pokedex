...
function PreviousNext({ previous, id, shiny }) {
  ...
  return (
    <Card
      ...
      className={
        previous
          ? `side border-dark previous ${
              loading ? `normal` : `${data.types[0].type.name}`
            }`
          : `side border-dark next ${
              loading ? `normal` : `${data.types[0].type.name}`
            }`
      }
    >
      {previous ? (
        <GrFormPrevious className='side__arrow' />
      ) : (
        <GrFormNext className='side__arrow' />
      )}
      <div className='side__pkmn'>
        <Card.Img
          src={pokeball}
          alt='Pokéball transparent'
          className='side__pkmn__pokeball'
        />
        {loading ? (
          <Spinner className='side__pkmn__loading' />
        ) : (
          <Card.Img
            variant='top'
      ...
      <Card.Body className={previous ? "side__body" : "side__body next__side"}>
        {loading ? (
          <Placeholder
            as={Card.Title}
            animation='glow'
            className='side__body__title'
          >
            <Placeholder md={12} />
          </Placeholder>
        ) : (
          <Card.Title className='side__body__title'>
            {`${data.name}`.charAt(0).toUpperCase()}
            {`${data.name}`.slice(1)}
          </Card.Title>
        )}
        <Card.Text className='side__body__id'>
